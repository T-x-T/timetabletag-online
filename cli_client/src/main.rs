...
fn make_move(game_id: &str, player_id: &str) -> String {
  ...
  let move_to_make = MakeMovePostBody {
    ...
    use_event_card: None,
    buy_powerup: None,
    throw_timetable_cards_away: vec![],
    finish_move: true,
  };

  println!("{move_to_make:?}");

  let move_result = ureq::post(format!("http://localhost:4000/api/v1/games/{game_id}/make_move"))
...
impl Display for InProgressGameState {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    return write!(f, "The runner is: {}\nCurrent turn: {}\nYour timetable cards: {}", self.runner, self.current_turn, self.your_timetable_cards.join(", "));
  }
}
