				...
				Some(game) => {
					let current_state_json = match game.state {
						...
						GameState::InProgress => serde_json::to_string(&InProgressGameState {
							...
							your_timetable_cards: game.timetable_cards.get(&query.player_id.unwrap_or_default()).unwrap_or(&Vec::new()).clone(),
							chaser_timetable_cards: game.timetable_cards.clone().into_iter().filter(|x| x.0 != game.runner.clone().unwrap().id).map(|x| (game.players.iter().filter(|y| y.id == x.0).next().unwrap().display_name.clone(), x.1)).collect(),
							last_used_timetable_card: game.last_used_timetable_card.clone().unwrap_or_default(),
